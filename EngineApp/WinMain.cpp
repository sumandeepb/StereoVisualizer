...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nShowCmd)
{
	...
	*stdout = *hf;
	...
	int nNumArgs = sscanf (pCmdLine, "%s%s", szConfigFileName, szAssetFileName);
	if (2 != nNumArgs)
		return -1;

	// load engine settings
	if (FALSE == g_cD3DSettings.loadD3DSettings (szConfigFileName))
		return -1;
	...
	while(msg.message != WM_QUIT)
	{
		...
		{	
			...
			g_cD3DEngine.AsyncKeyDown (0.075f);
...
bool InitWindowsApp(HINSTANCE instanceHandle, int show)
{
	...
	g_hMainWnd = CreateWindow(
		...
		g_cD3DSettings.getWidth(),	// width
		g_cD3DSettings.getHeight(),	// height
		0,					// parent window
		0,					// menu handle
