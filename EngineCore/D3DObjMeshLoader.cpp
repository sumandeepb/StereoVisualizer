...
HRESULT CD3DObjMeshLoader::Create (const std::string &strFilename, IDirect3DDevice9* pd3dDevice, ID3DXMesh** ppMesh, float rScale)
{
	...
	CD3DModel::SVertex *aVertexBuff = NULL;
	pMesh->LockVertexBuffer(0, (void**)&aVertexBuff);
	for (unsigned int i = 0; i < m_aVertices.size(); i++)
	{
		...
		sTempVertex._nx = -m_aVertices [i].vNormal.x; // D3D uses LH coordinate system
		sTempVertex._ny = m_aVertices [i].vNormal.y;
		sTempVertex._nz = m_aVertices [i].vNormal.z;
		aVertexBuff[i] = sTempVertex;
	}
	pMesh->UnlockVertexBuffer();

	// load index buffer
	DWORD* aIndexBuff = NULL;
	pMesh->LockIndexBuffer(0, (void**)&aIndexBuff);
	for (unsigned int i = 0; i < m_aIndices.size(); i++)
	{
		aIndexBuff[i] = m_aIndices[i];
	}
	pMesh->UnlockIndexBuffer();

	// load attribute buffer
	DWORD* aAttribBuff = 0;
	pMesh->LockAttributeBuffer(0, &aAttribBuff);
	for (unsigned int i = 0; i < m_aAttributes.size(); i++)
	{
		aAttribBuff [i] = m_aAttributes[i];
	}
	...
	// compute the normals:
	//D3DXComputeNormals (pMesh, /* m_optimizedAdjacencyInfo*/ NULL);
...
HRESULT CD3DObjMeshLoader::LoadGeometryFromOBJ (const std::string &strFilename)
{
	...
	while (1)
	{
		...
		else if( 0 == strcmp( szCommand, "usemtl" ) )
		{
			...
			if( !bFound )
			{
				if (m_aIndices.size() > 0)	// there may be triangles without material definition
					// they will use the default material
					// therefore add new material
				{
					SMaterial sMaterial;
					InitMaterial (sMaterial);
					m_aMaterials.push_back (sMaterial);
...
HRESULT CD3DObjMeshLoader::LoadMaterialsFromMTL (const std::string &strFilename)
{
	...
	while (1)
	{
		...
		else if( 0 == strcmp( szCommand, "illum" ) )
		{
			...
			m_aMaterials[iMaterial].nIllumMode = illumination;
		}
...
void CD3DObjMeshLoader::InitMaterial (SMaterial &sMaterial)
{
	...
	sMaterial.rShininess = 2.0f;
	sMaterial.rAlpha = 1.0f;
	sMaterial.nIllumMode = 2;
