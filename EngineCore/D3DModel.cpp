...
BOOL CD3DModel::load3DModel (IDirect3DDevice9* pD3DDevice, const std::string &strModelFile, float rScale)
{
	...
	if(uExtPos != std::string::npos && strExtension == "obj")
	{
		...
		//unsigned int nNumMaterials = cObjMeshLoader.GetNumMaterials ();
		...
		for (unsigned int index = 0; index < nNumSubsets; index++)
		{
			...
			//{
			//	sMaterial.vAmbient = 0.2f * sMaterial.vDiffuse;
			//}
			sMaterial.rAlpha = 1.0f;
			m_asMaterial[index] = initMtrl (vectorToColor (sMaterial.vAmbient, sMaterial.rAlpha), 
				vectorToColor (sMaterial.vDiffuse, sMaterial.rAlpha), 
				vectorToColor (sMaterial.vSpecular, sMaterial.rAlpha), 
				vectorToColor (sMaterial.vEmissive, sMaterial.rAlpha), sMaterial.rShininess);
			if ( sMaterial.strTexture != "" )
			{            
				if (FAILED (D3DXCreateTextureFromFile (pD3DDevice, 
					sMaterial.strTexture.c_str(), &m_apTexture[index])))
				{
					return FALSE;
				}
#ifdef _DEBUG
...
void CD3DModel::setFillMode (D3DFILLMODE eFillMode)
{
...
BOOL CD3DModel::renderModel ()
{
	if (m_pD3DDevice)
	{
		// set fill mode (points / wireframe / solid)
		m_pD3DDevice->SetRenderState (D3DRS_FILLMODE, m_eFillMode);
		...
		for (unsigned int index = 0; index < m_asMaterial.size(); index++)
		{
			...
			if (m_pObjectMesh)
			{
				m_pObjectMesh->DrawSubset (index);
			}
		}
	}
